  ...
  private setupHandlers(): void {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        {
          name: 'list_prompts',
          description: 'List all available prompts with brief descriptions. Use if you did not find prompts by search_prompts',
          inputSchema: {
            type: 'object',
            properties: {},
          },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          case 'get_prompt':
            return buildTextResponse(JSON.stringify(await this.getPrompt(args?.filename as string), null, 2))
          ...
          case 'search_prompts':
            return buildTextResponse(JSON.stringify(await this.searchPrompts(args?.query as string), null, 2))

          default:
            throw new Error(`Unknown tool: ${name}`);
        }
      } catch (error) {
        return buildTextResponse(JSON.stringify(
          {
