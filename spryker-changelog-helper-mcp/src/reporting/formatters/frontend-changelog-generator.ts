...
export class FrontendChangelogGenerator {
  generate(changes: FrontendChange[]): FrontendChangelogEntry[] {
    const entries: FrontendChangelogEntry[] = [];
  ...
  private generateEntriesForChange(change: FrontendChange): FrontendChangelogEntry[] {
    const entries: FrontendChangelogEntry[] = [];
  ...
  private generateAddedComponentEntries(change: FrontendChange, componentRef: string): FrontendChangelogEntry[] {
    const entries: FrontendChangelogEntry[] = [];
  ...
  private generateDeletedComponentEntries(change: FrontendChange, componentRef: string): FrontendChangelogEntry[] {
    const entries: FrontendChangelogEntry[] = [];
  ...
  private generateModifiedComponentEntries(change: FrontendChange, componentRef: string): FrontendChangelogEntry[] {
    const entries: FrontendChangelogEntry[] = [];
  ...
  private formatComponentReference(name: string, type: FrontendChange['componentType']): string {
    if (type === 'atom') return `\`${name}\` atom`;
    if (type === 'molecule') return `\`${name}\` molecule`;
    if (type === 'organism') return `\`${name}\` organism`;
    if (type === 'template') return `\`${name}\` template`;
    if (type === 'page') return `\`${name}\` page`;
    return `\`${name}\``;
  }
}
