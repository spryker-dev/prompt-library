    ...
    try {
        ...
        if (name === 'analyze_spryker_changes') {
            ...
            if (args.modulePatterns) {
                const fs = require('fs');
                const patternsData = fs.readFileSync(args.modulePatterns as string, 'utf-8');
                modulePatterns = JSON.parse(patternsData);
            }

            const service = new ImpactAnalysisService(modulePatterns);
            ...
            if (args.modules) {
                const requestedModules = new Set((args.modules as string).split(',').map(m => m.trim()));
