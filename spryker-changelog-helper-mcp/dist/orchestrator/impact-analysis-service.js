...
const impact_report_generator_1 = require("./impact-report-generator");
...
class ImpactAnalysisService {
    constructor(modulePatterns) {
        this.modulePatterns = modulePatterns || analyzer_config_1.AnalyzerConfig.defaultModulePatterns;
    }
            ...
            for (const classChange of diffResult.newClasses || []) {
                ...
                for (const [fqcn, classMetadata] of symbolIndex.classes.entries()) {
                    if (classMetadata.file.endsWith(classChange.file) || classChange.file.endsWith(classMetadata.file)) {
                        ...
                        classChange.name = fqcn.split(analyzer_config_1.AnalyzerConfig.spryker.namespaceSeparator).pop() || '';
                        break;
                    }
                }
            }
    ...
    async analyzeModuleImpact(root, moduleName, methods, modulePatterns, impactResults, logger) {
        ...
        try {
            const analyzer = new impact_analyzer_1.ImpactAnalyzer({
                ...
                excludes: [],
                targets,
                entrypointRegex: new RegExp(analyzer_config_1.AnalyzerConfig.getEntrypointPattern(), 'i'),
                linkInterfaces: true,
                debug: false,
                logger,
            });
