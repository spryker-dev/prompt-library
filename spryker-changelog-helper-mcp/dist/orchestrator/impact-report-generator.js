...
const public_api_detector_1 = require("./utils/public-api-detector");
const version_recommender_1 = require("./version-recommender");
const module_report_generator_1 = require("./module-report-generator");
...
class ImpactReportGenerator {
    ...
    isConfigMethod(method) {
        if (method.visibility !== analyzer_config_1.AnalyzerConfig.visibility.public)
            return false;
        return /\\Config(::|$)/.test(method.fqcn) || method.file.includes(analyzer_config_1.AnalyzerConfig.filePatterns.config);
    }
    isCommunicationLayerAdjustment(method) {
        // Only track public/protected methods in Communication layer
        if (method.visibility === analyzer_config_1.AnalyzerConfig.visibility.private)
            return false;
        // Check if it's in Communication layer
        const communicationLayer = analyzer_config_1.AnalyzerConfig.spryker.internalApiLayers.find(layer => layer === 'Communication');
        if (!method.file.includes(`/${communicationLayer}/`))
            return false;
        const className = method.fqcn.split('::')[0];
        const simpleClassName = className.split(analyzer_config_1.AnalyzerConfig.spryker.namespaceSeparator).pop() || '';
        // Check against configured Communication layer class types
        const { controller, form, table } = analyzer_config_1.AnalyzerConfig.spryker.communicationLayerClasses;
    ...
    calculateRiskLevel(modifiedPublicAPI, newPublicAPI, modifiedConfigMethods, internalWithImpact) {
        if (modifiedPublicAPI > 0 || modifiedConfigMethods > analyzer_config_1.AnalyzerConfig.riskThresholds.modifiedConfigMethodsHigh) {
            return analyzer_config_1.AnalyzerConfig.riskLevels.high;
        }
        if (newPublicAPI > analyzer_config_1.AnalyzerConfig.riskThresholds.newPublicAPIMedium ||
            internalWithImpact > analyzer_config_1.AnalyzerConfig.riskThresholds.internalWithImpactMedium ||
            modifiedConfigMethods > 0) {
            return analyzer_config_1.AnalyzerConfig.riskLevels.medium;
        }
        return analyzer_config_1.AnalyzerConfig.riskLevels.low;
    }
}
