...
class SchemaAnalyzer {
    ...
    extractTableName(diff) {
        const match = diff.match(new RegExp(`${analyzer_config_1.AnalyzerConfig.schemaTags.table}"([^"]+)"`));
    ...
    extractAddedColumns(diff) {
        ...
        for (const line of lines) {
            if (line.startsWith(analyzer_config_1.AnalyzerConfig.diffPrefixChars.added) && line.includes(analyzer_config_1.AnalyzerConfig.schemaTags.column)) {
    ...
    extractRemovedColumns(diff) {
        ...
        const lines = diff.split('\n');
        for (const line of lines) {
            if (line.startsWith(analyzer_config_1.AnalyzerConfig.diffPrefixChars.removed) && line.includes(analyzer_config_1.AnalyzerConfig.schemaTags.column)) {
                const column = this.parseColumnLine(line.substring(1));
                if (column) {
                    columns.push(column);
                }
    ...
    extractAddedIndexes(diff) {
        ...
        for (let i = 0; i < lines.length; i++) {
            if (lines[i].startsWith(analyzer_config_1.AnalyzerConfig.diffPrefixChars.added) && lines[i].includes(analyzer_config_1.AnalyzerConfig.schemaTags.indexName)) {
    ...
    extractRemovedIndexes(diff) {
        ...
        const lines = diff.split('\n');
        for (let i = 0; i < lines.length; i++) {
            if (lines[i].startsWith(analyzer_config_1.AnalyzerConfig.diffPrefixChars.removed) && lines[i].includes(analyzer_config_1.AnalyzerConfig.schemaTags.indexName)) {
                const index = this.parseIndexBlock(lines, i);
                if (index) {
                    indexes.push(index);
                }
    ...
    parseColumnLine(line) {
        ...
        return {
            ...
            required: requiredMatch ? requiredMatch[1] === analyzer_config_1.AnalyzerConfig.xmlAttributes.trueValue : undefined,
    ...
    parseIndexBlock(lines, startIndex) {
        ...
        for (let i = startIndex + 1; i < lines.length; i++) {
            ...
            if (currentLine.includes(analyzer_config_1.AnalyzerConfig.schemaTags.indexClose)) {
                break;
            }
            if (currentLine.includes(analyzer_config_1.AnalyzerConfig.schemaTags.indexColumn)) {
        ...
        return {
            name: nameMatch[1],
            columns,
            unique: uniqueMatch ? uniqueMatch[1] === analyzer_config_1.AnalyzerConfig.xmlAttributes.trueValue : undefined,
        };
    }
}
