...
class TransferAnalyzer {
    ...
    analyzeTransferFile(status, file, baseCommit) {
        if (status.startsWith(analyzer_config_1.AnalyzerConfig.gitStatus.added)) {
            return this.handleAddedTransferFile(file);
        }
        if (status.startsWith(analyzer_config_1.AnalyzerConfig.gitStatus.deleted)) {
    ...
    compareTransfers(file, oldTransfers, newTransfers) {
        ...
        for (const [name, newTransfer] of newTransfers) {
            if (!oldTransfers.has(name)) {
                changes.push({ file, transferName: name, changeType: analyzer_config_1.AnalyzerConfig.changeTypes.new });
                continue;
            }
        ...
        for (const [name] of oldTransfers) {
            if (!newTransfers.has(name)) {
                changes.push({ file, transferName: name, changeType: analyzer_config_1.AnalyzerConfig.changeTypes.removed });
            }
        }
        return changes;
    }
    ...
    extractTransferNamesFromFile(file) {
        try {
            ...
            const matches = content.matchAll(analyzer_config_1.AnalyzerConfig.xmlPatterns.transferOpen);
