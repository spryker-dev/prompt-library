...
class VersionRecommender {
    recommend(report) {
        ...
        if (report.removedMethods.length > 0) {
            const publicRemovals = report.removedMethods.filter(m => analyzer_config_1.AnalyzerConfig.spryker.publicApiClassSuffixes.some(suffix => m.fqcn.includes(suffix)));
        ...
        const removedSchemaChanges = report.schemaChanges.filter(s => s.changeType === analyzer_config_1.AnalyzerConfig.changeTypes.removed ||
        ...
        if (removedSchemaChanges.length > 0) {
            ...
            if (removedColumns > 0 || removedSchemaChanges.some(s => s.changeType === analyzer_config_1.AnalyzerConfig.changeTypes.removed)) {
                reasons.push({
                    type: 'MAJOR',
                    category: 'Database Breaking Changes',
                    description: 'Database columns/tables removed',
                    count: removedColumns + removedSchemaChanges.filter(s => s.changeType === analyzer_config_1.AnalyzerConfig.changeTypes.removed).length,
                });
            }
        }
        // Check for removed transfers
        const removedTransfers = report.transferChanges.filter(t => t.changeType === analyzer_config_1.AnalyzerConfig.changeTypes.removed ||
        ...
        if (removedTransfers.length > 0) {
            ...
            reasons.push({
                ...
                count: removedProps + removedTransfers.filter(t => t.changeType === analyzer_config_1.AnalyzerConfig.changeTypes.removed).length,
            });
        }
        ...
        const newSchemaChanges = report.schemaChanges.filter(s => s.changeType === analyzer_config_1.AnalyzerConfig.changeTypes.new ||
        ...
        if (newSchemaChanges.length > 0) {
            ...
            reasons.push({
                ...
                count: addedColumns + newSchemaChanges.filter(s => s.changeType === analyzer_config_1.AnalyzerConfig.changeTypes.new).length,
            });
        }
        // New transfers
        const newTransfers = report.transferChanges.filter(t => t.changeType === analyzer_config_1.AnalyzerConfig.changeTypes.new ||
        ...
        if (newTransfers.length > 0) {
            ...
            reasons.push({
                ...
                count: addedProps + newTransfers.filter(t => t.changeType === analyzer_config_1.AnalyzerConfig.changeTypes.new).length,
            });
        }
        ...
        const hasMajor = reasons.some(r => r.type === analyzer_config_1.AnalyzerConfig.versionBumpTypes.major);
        const hasMinor = reasons.some(r => r.type === analyzer_config_1.AnalyzerConfig.versionBumpTypes.minor);
        let recommendedBump;
        let confidence;
        if (hasMajor) {
        ...
        if (report.modifiedInternalFiles && report.modifiedInternalFiles.length > 0) {
            const signatureChanges = report.modifiedInternalFiles.filter(f => f.changeType === analyzer_config_1.AnalyzerConfig.internalChangeTypes.signature).length;
        ...
        if (report.nonPhpFiles.length > 0) {
            const composerChanges = report.nonPhpFiles.filter(f => f.file.includes(analyzer_config_1.AnalyzerConfig.filePatterns.composerJson));
            ...
            const configChanges = report.nonPhpFiles.filter(f => f.file.includes(analyzer_config_1.AnalyzerConfig.filePatterns.configFile) || f.file.includes(analyzer_config_1.AnalyzerConfig.filePatterns.yamlExt) || f.file.includes(analyzer_config_1.AnalyzerConfig.filePatterns.yamlExtAlt));
