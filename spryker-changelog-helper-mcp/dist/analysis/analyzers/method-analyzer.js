...
class MethodAnalyzer {
    ...
    processBlock(file, className, block, methodSignatures, result) {
        ...
        if (changeType === change_constants_1.ChangeType.MODIFIED) {
    ...
    processBlockWithoutMethodContext(file, className, block, methodSignatures, result) {
        ...
        for (const line of block.lines) {
            const cleanLine = this.removeDiffPrefix(line);
            const methodMatch = cleanLine.match(php_constants_1.PhpPattern.METHOD_SIGNATURE);
            if (methodMatch) {
    ...
    processSignatureLine(line, tracker) {
        const methodMatch = line.match(php_constants_1.PhpPattern.DIFF_METHOD_SIGNATURE);
        if (!methodMatch)
            return;
    ...
    updateMethodSignature(methodName, prefix, tracker) {
        ...
        if (prefix === git_constants_1.DiffPrefixes.ADDITION) {
            signature.added = true;
        }
        if (prefix === git_constants_1.DiffPrefixes.REMOVAL) {
    ...
    findMethodNameInChanges(block) {
        ...
        for (const line of [...block.additions, ...block.removals]) {
            const methodMatch = line.match(php_constants_1.PhpPattern.METHOD_SIGNATURE);
            if (methodMatch)
    ...
    determineChangeType(methodName, block, tracker) {
        ...
        return change_constants_1.ChangeType.NONE;
    }
    ...
    isAddition(line) {
        return line.startsWith(git_constants_1.DiffPrefixes.ADDITION) &&
            !line.startsWith(git_constants_1.DiffPrefixes.ADDITION_FILE);
    }
    isRemoval(line) {
        return line.startsWith(git_constants_1.DiffPrefixes.REMOVAL) &&
            !line.startsWith(git_constants_1.DiffPrefixes.REMOVAL_FILE);
    }
}
