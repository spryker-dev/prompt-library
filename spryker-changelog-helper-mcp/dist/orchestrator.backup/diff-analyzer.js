...
const schema_analyzer_1 = require("../analysis/analyzers/schema-analyzer");
...
class DiffAnalyzer {
    constructor(root, baseCommit, includeTests, _logger, modules) {
        ...
        this.gitHelper = new git_operations_1.GitHelper(root, modules);
    ...
    analyzePhpFile(file, status, baseCommit, result) {
        try {
            if (status === module_patterns_1.AnalyzerConfig.gitStatus.added) {
    ...
    extractLayer(file) {
        const layers = module_patterns_1.AnalyzerConfig.spryker.layers;
        const internalLayers = module_patterns_1.AnalyzerConfig.spryker.internalApiLayers;
    ...
    createSkippedFileForEmptyDiff(file) {
        return {
            file,
            reason: module_patterns_1.AnalyzerConfig.skipReasons.emptyDiff,
    ...
    createSkippedFileForError(file, error) {
        return {
            file,
            reason: error.message || module_patterns_1.AnalyzerConfig.skipReasons.unknownError,
            error: error.stack,
