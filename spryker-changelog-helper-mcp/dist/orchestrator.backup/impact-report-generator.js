...
class ImpactReportGenerator {
    ...
    isConfigMethod(method) {
        if (method.visibility !== module_patterns_1.AnalyzerConfig.visibility.public)
            return false;
        return /\\Config(::|$)/.test(method.fqcn) || method.file.includes(module_patterns_1.AnalyzerConfig.filePatterns.config);
    }
    isCommunicationLayerAdjustment(method) {
        // Only track public/protected methods in Communication layer
        if (method.visibility === module_patterns_1.AnalyzerConfig.visibility.private)
            return false;
        // Check if it's in Communication layer
        const communicationLayer = module_patterns_1.AnalyzerConfig.spryker.internalApiLayers.find(layer => layer === 'Communication');
        if (!method.file.includes(`/${communicationLayer}/`))
            return false;
        const className = method.fqcn.split('::')[0];
        const simpleClassName = className.split(module_patterns_1.AnalyzerConfig.spryker.namespaceSeparator).pop() || '';
        // Check against configured Communication layer class types
        const { controller, form, table } = module_patterns_1.AnalyzerConfig.spryker.communicationLayerClasses;
    ...
    calculateRiskLevel(modifiedPublicAPI, newPublicAPI, modifiedConfigMethods, internalWithImpact) {
        if (modifiedPublicAPI > 0 || modifiedConfigMethods > module_patterns_1.AnalyzerConfig.riskThresholds.modifiedConfigMethodsHigh) {
            return module_patterns_1.AnalyzerConfig.riskLevels.high;
        }
        if (newPublicAPI > module_patterns_1.AnalyzerConfig.riskThresholds.newPublicAPIMedium ||
            internalWithImpact > module_patterns_1.AnalyzerConfig.riskThresholds.internalWithImpactMedium ||
            modifiedConfigMethods > 0) {
            return module_patterns_1.AnalyzerConfig.riskLevels.medium;
        }
        return module_patterns_1.AnalyzerConfig.riskLevels.low;
    }
}
