...
class ChangelogFormatter {
    ...
    formatModuleChangelog(moduleReport) {
        ...
        for (const method of moduleReport.publicApiChanges.removedMethods) {
            breakingChanges.push({
                type: module_patterns_1.AnalyzerConfig.changelogTypes.method,
                action: module_patterns_1.AnalyzerConfig.changelogActions.removed,
        ...
        for (const method of moduleReport.publicApiChanges.modifiedMethods) {
            if (method.signatureChanged) {
                breakingChanges.push({
                    type: module_patterns_1.AnalyzerConfig.changelogTypes.method,
                    action: module_patterns_1.AnalyzerConfig.changelogActions.modified,
                    item: this.extractMethodName(method.method),
        ...
        for (const method of moduleReport.publicApiChanges.affectedByInternal) {
            improvements.push({
                type: module_patterns_1.AnalyzerConfig.changelogTypes.method,
                action: module_patterns_1.AnalyzerConfig.changelogActions.improved,
                item: this.extractMethodName(method.method),
                context: method.description || method.isApiMethod || method.isDeprecated ? {
                    description: method.description,
                    isApiMethod: method.isApiMethod,
        ...
        for (const method of moduleReport.publicApiChanges.newMethods) {
            improvements.push({
                type: module_patterns_1.AnalyzerConfig.changelogTypes.method,
                action: module_patterns_1.AnalyzerConfig.changelogActions.added,
                item: this.extractMethodName(method.method),
        ...
        for (const method of moduleReport.configChanges.newMethods) {
            improvements.push({
                type: module_patterns_1.AnalyzerConfig.changelogTypes.config,
                action: module_patterns_1.AnalyzerConfig.changelogActions.added,
                item: this.extractMethodName(method.method),
                visibility: method.visibility,
                file: method.file,
        ...
        for (const plugin of moduleReport.pluginChanges.affectedByInternal) {
            improvements.push({
                type: module_patterns_1.AnalyzerConfig.changelogTypes.plugin,
                action: module_patterns_1.AnalyzerConfig.changelogActions.improved,
        ...
        for (const pluginClass of moduleReport.pluginChanges.deprecatedPluginClasses) {
            deprecations.push({
                type: module_patterns_1.AnalyzerConfig.changelogTypes.plugin,
                action: module_patterns_1.AnalyzerConfig.changelogActions.deprecated,
                item: pluginClass.name,
                file: pluginClass.file,
            });
        }
        ...
        for (const constant of moduleReport.constantChanges.removedConstants) {
            const entry = {
                type: module_patterns_1.AnalyzerConfig.changelogTypes.constant,
                action: module_patterns_1.AnalyzerConfig.changelogActions.removed,
        ...
        for (const schema of moduleReport.dataChanges.schemas) {
            if (schema.addedColumns && schema.addedColumns.length > 0) {
                for (const column of schema.addedColumns) {
                    improvements.push({
                        type: module_patterns_1.AnalyzerConfig.changelogTypes.schema,
                        action: module_patterns_1.AnalyzerConfig.changelogActions.added,
        ...
        for (const method of moduleReport.communicationChanges.adjustedMethods) {
            ...
            if (isController) {
                ...
                adjustments.push({
                    type: module_patterns_1.AnalyzerConfig.changelogTypes.method,
                    action: module_patterns_1.AnalyzerConfig.changelogActions.adjusted,
                    item: this.extractMethodName(method.method),
                    file: method.file,
                });
            }
        ...
        for (const className of nonControllerClasses) {
            adjustments.push({
                type: module_patterns_1.AnalyzerConfig.changelogTypes.class,
                action: module_patterns_1.AnalyzerConfig.changelogActions.adjusted,
                item: className,
        ...
        if (moduleReport.composerChanges) {
            for (const dep of moduleReport.composerChanges.dependencyChanges) {
                adjustments.push({
                    type: module_patterns_1.AnalyzerConfig.changelogTypes.composer,
                    action: module_patterns_1.AnalyzerConfig.changelogActions.modified,
                    item: dep.package,
        ...
        for (const transfer of moduleReport.dataChanges.transfers) {
            if (transfer.changeType === module_patterns_1.AnalyzerConfig.changeTypes.new) {
                transfers.push({
                    type: module_patterns_1.AnalyzerConfig.changelogTypes.transfer,
                    action: module_patterns_1.AnalyzerConfig.changelogActions.added,
                    item: transfer.name,
                });
                continue;
    ...
    generateSummary(modules) {
        ...
        for (const moduleData of modules.values()) {
            if (moduleData.versionBump === module_patterns_1.AnalyzerConfig.versionBumpTypes.major)
                majorReleases++;
            else if (moduleData.versionBump === module_patterns_1.AnalyzerConfig.versionBumpTypes.minor)
                minorReleases++;
            else
                patchReleases++;
        }
        const overallRisk = majorReleases > 0
            ? module_patterns_1.AnalyzerConfig.riskLevels.high.level
            : minorReleases > 0
                ? module_patterns_1.AnalyzerConfig.riskLevels.medium.level
                : module_patterns_1.AnalyzerConfig.riskLevels.low.level;
        return {
