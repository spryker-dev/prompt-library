...
class PHPParser {
    extractClassNameFromFilePath(file) {
        const match = file.match(module_patterns_1.AnalyzerConfig.patterns.classNameFromPath);
        return match ? match[1] : null;
    }
    buildFQCN(file, className) {
        const fullMatch = file.match(module_patterns_1.AnalyzerConfig.patterns.sprykerFilePath);
        if (!fullMatch) {
    ...
    convertPathToNamespace(path) {
        const { pathSeparator, namespaceSeparator } = module_patterns_1.AnalyzerConfig.spryker;
    ...
    buildSprykerFQCN(vendor, layer, module, namespacePath, className) {
        const { namespaceSeparator } = module_patterns_1.AnalyzerConfig.spryker;
    ...
    buildGenericFQCN(className) {
        return module_patterns_1.AnalyzerConfig.spryker.namespaceSeparator + className;
    }
    extractMethodNameFromLine(line) {
        const match = line.match(module_patterns_1.AnalyzerConfig.patterns.methodSignature);
        return match ? match[1] : null;
    }
    extractMethodDetails(line) {
        ...
        const isStatic = line.includes(module_patterns_1.AnalyzerConfig.phpKeywords.static);
    ...
    isMethodSignatureLine(line) {
        return module_patterns_1.AnalyzerConfig.patterns.methodSignature.test(line);
    }
}
