...
class FileFilter {
    ...
    filterPhpFiles(gitStatusLines) {
        const phpFiles = gitStatusLines
            ...
            const file = parts[1] || '';
            return file.endsWith(module_patterns_1.AnalyzerConfig.fileExtensions.php);
        })
            .map((line) => {
            const parts = line.split(/\s+/);
            return parts[1];
        ...
        return this.includeTests
            ? phpFiles
            : phpFiles.filter((f) => !module_patterns_1.AnalyzerConfig.patterns.testDirectory.test(f));
    }
    filterPhpFilesWithStatus(gitStatusLines) {
        ...
        return this.includeTests
            ? phpFiles
            : phpFiles.filter((item) => !module_patterns_1.AnalyzerConfig.patterns.testDirectory.test(item.file));
    }
    identifyNonPhpFiles(gitStatusLines, mapGitStatus) {
        ...
        for (const line of gitStatusLines) {
            ...
            const file = parts[1];
            if (!file || file.endsWith(module_patterns_1.AnalyzerConfig.fileExtensions.php))
                continue;
            const hasRelevantExtension = module_patterns_1.AnalyzerConfig.relevantNonPhpExtensions.some((ext) => file.endsWith(ext));
