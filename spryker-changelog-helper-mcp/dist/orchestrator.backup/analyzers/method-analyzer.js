...
class MethodAnalyzer {
    ...
    createSkippedFileForNoClassName(file) {
        return {
            file,
            reason: module_patterns_1.AnalyzerConfig.skipReasons.noClassName,
    ...
    processBlock(file, className, block, methodSignatures, result) {
        ...
        if (changeType === module_patterns_1.AnalyzerConfig.changeTypes.modified) {
    ...
    processBlockWithoutMethodContext(file, className, block, methodSignatures, result) {
        ...
        for (const line of block.lines) {
            const cleanLine = this.removeDiffPrefix(line);
            const methodMatch = cleanLine.match(module_patterns_1.AnalyzerConfig.patterns.methodSignature);
            if (methodMatch) {
            ...
            if (cleanLine.match(module_patterns_1.AnalyzerConfig.patterns.closingBrace)) {
                currentMethod = null;
            }
    ...
    processSignatureLine(line, tracker) {
        const methodMatch = line.match(module_patterns_1.AnalyzerConfig.patterns.diffMethodSignature);
        if (!methodMatch)
            return;
    ...
    updateMethodSignature(methodName, prefix, tracker) {
        ...
        if (prefix === module_patterns_1.AnalyzerConfig.diffPrefixes.addition) {
            signature.added = true;
        }
        if (prefix === module_patterns_1.AnalyzerConfig.diffPrefixes.removal) {
    ...
    findMethodNameInChanges(block) {
        ...
        for (const line of [...block.additions, ...block.removals]) {
            const methodMatch = line.match(module_patterns_1.AnalyzerConfig.patterns.methodSignature);
            if (methodMatch)
    ...
    extractMethodDetails(block) {
        for (const line of block.lines) {
            ...
            if (!methodMatch)
                continue;
            const visibility = line.match(module_patterns_1.AnalyzerConfig.patterns.visibilityModifier)?.[1] || module_patterns_1.AnalyzerConfig.visibilityModifiers.public;
            return { visibility, params: '' };
        }
        return { visibility: module_patterns_1.AnalyzerConfig.visibilityModifiers.public, params: '' };
    }
    determineChangeType(methodName, block, tracker) {
        ...
        return module_patterns_1.AnalyzerConfig.changeTypes.none;
    }
    ...
    isBlockHeader(line) {
        return line.startsWith(module_patterns_1.AnalyzerConfig.diffBlockMarker);
    }
    ...
    removeDiffPrefix(line) {
        const prefixes = [module_patterns_1.AnalyzerConfig.diffPrefixChars.added, module_patterns_1.AnalyzerConfig.diffPrefixChars.removed, module_patterns_1.AnalyzerConfig.diffPrefixChars.unchanged];
        return prefixes.some(p => line.startsWith(p)) ? line.slice(1) : line;
    }
    isAddition(line) {
        return line.startsWith(module_patterns_1.AnalyzerConfig.diffPrefixes.addition) &&
            !line.startsWith(module_patterns_1.AnalyzerConfig.diffPrefixes.additionFileMarker);
    }
    isRemoval(line) {
        return line.startsWith(module_patterns_1.AnalyzerConfig.diffPrefixes.removal) &&
            !line.startsWith(module_patterns_1.AnalyzerConfig.diffPrefixes.removalFileMarker);
    }
}
